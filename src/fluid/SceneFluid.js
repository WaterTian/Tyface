...
var fsAdvect = glslify('./shaders/advect.frag');
var fsDivergence = glslify('./shaders/divergence.frag');
var fsClear = glslify('./shaders/clear.frag');
var fsJacobi = glslify('./shaders/jacobi.frag');
var fsGradientSub = glslify('./shaders/gradientSubstract.frag');
var fsSplat = glslify('./shaders/splat.frag');
var fsNormal = glslify('./shaders/normal.frag');
var fsPost = glslify('./shaders/post.frag');


export default class SceneFluid extends Scene {
	constructor() {
		super();
		this.resize();
	...
	_initTextures() {
		...
		this._fboVelocity = new FboPingPong(width, height, {
			minFilter: GL.LINEAR,
			magFilter: GL.LINEAR,
			type
		});
		this._fboDensity = new FboPingPong(width, height, {
			minFilter: GL.LINEAR,
			magFilter: GL.LINEAR,
			type
		});
		this._fboPressure = new FboPingPong(width, height, {
			minFilter: GL.LINEAR,
			magFilter: GL.LINEAR,
			type
		});
		this._fboDivergence = new alfrid.FrameBuffer(width, height, {
			minFilter: GL.NEAREST,
			magFilter: GL.NEAREST,
			type
		});
		this._fboNormal = new alfrid.FrameBuffer(width, height, {
			minFilter: GL.LINEAR,
			magFilter: GL.LINEAR
		});
	}
